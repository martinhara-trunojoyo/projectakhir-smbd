...
if (isset($_POST['Simpan'])) {
    ...
    try {
        ...
        $sql_simpan = "CALL AddPendudukBaru(
            :nik, :nama, :tempat_lh, :tgl_lh, :jekel, :desa, :rt, :rw, :agama, :kawin, :pekerjaan)";
        
        // Menyiapkan statement
        $stmt = $conn->prepare($sql_simpan);

        // Bind parameter
        $stmt->bindParam(':nik', $_POST['nik']);
        $stmt->bindParam(':nama', $_POST['nama']);
        $stmt->bindParam(':tempat_lh', $_POST['tempat_lh']);
        $stmt->bindParam(':tgl_lh', $_POST['tgl_lh']);
        $stmt->bindParam(':jekel', $_POST['jekel']);
        $stmt->bindParam(':desa', $_POST['desa']);
        $stmt->bindParam(':rt', $_POST['rt']);
        $stmt->bindParam(':rw', $_POST['rw']);
        $stmt->bindParam(':agama', $_POST['agama']);
        $stmt->bindParam(':kawin', $_POST['kawin']);
        ...
        echo "<script>
        Swal.fire({title: 'Tambah Data Gagal', text: '".addslashes($e->getMessage())."', icon: 'error', confirmButtonText: 'OK'
        }).then((result) => {
            if (result.value) {
